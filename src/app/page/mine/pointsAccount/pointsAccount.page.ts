...
@Component({
  selector: 'app-points-account',
  templateUrl: './pointsAccount.page.html',
  styleUrls: ['./pointsAccount.page.scss'],
})
export class PointsAccountPage implements OnInit {
  ...
  constructor(
    ...
    private native: NativeService,
      ...
      // this.totalAmount = globalData.user.pay_points;
  }
  ...
  // 初始化页面
  ionViewWillEnter() {
    this.totalAmount = this.globalData.user.pay_points;
    // 初始化查询结束时间
    let now = new Date();
      ...
      if(res.status == 200){
        if(res.data.result.length > 0){
          ...
          for(let item of res.data.result){
            item.pay_points = Number(item.pay_points);
          }
