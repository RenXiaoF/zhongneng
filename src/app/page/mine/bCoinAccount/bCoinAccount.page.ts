...
@Component({
  selector: 'app-bcoin-account',
  templateUrl: './bCoinAccount.page.html',
  styleUrls: ['./bCoinAccount.page.scss'],
})
export class BCoinAccountPage implements OnInit {
  public data:boolean = false;  // 保存订单明细是否有数据，默认false为没有数据
  public totalBcoin = 0; // 总批发券数
  ...
  constructor(
    ...
    private native: NativeService,
      ...
      // this.totalBcoin = globalData.user.b_coin;
  }
  ...
  // 初始化页面
  ionViewWillEnter() {
    this.totalBcoin = this.globalData.user.b_coin;
    // 初始化查询结束时间
    let now = new Date();
      ...
      if(res.status == 200){
        if(res.data.result.length > 0){
          ...
          for(let item of res.data.result){
            item.b_coin = Number(item.b_coin);
          }
